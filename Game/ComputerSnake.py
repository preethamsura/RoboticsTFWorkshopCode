...
class Snake(py_environment.PyEnvironment):
    ...
    def __init__(self, persistence = False, evaluation = False):
    ...
    def _step(self, action):
        ...
        if ((not self.checkLose()) and (self.curr_moves < self.move_limit)):
            ...
            return ts.transition(self._state, reward, discount=.98)
    ...
    def checkLose(self):
        ...
        if (not convertToKey(head) in self.openLocations or self.curr_moves >= (self.score + 1) * 1000):
            self.dead = True
            return True
